...
def make_Face_transforms(image_set):
    ...
    scales = [720, 768, 816, 864, 912, 960, 1008, 1056, 1104, 1152, 1200]
    if image_set == 'train':
        return T.Compose([
            T.RandomHorizontalFlip(),
            T.RandomSelect(
                T.RandomResize(scales, max_size=1998),
                T.Compose([
                    T.RandomResize([600, 750, 900]),
                    T.RandomSizeCrop(594, 900),
                    T.RandomResize(scales, max_size=1998),
                ])
            ),
            normalize,
    ...
    if image_set == 'val':
        return T.Compose([
            T.RandomResize([1200], max_size=1998),
            normalize,
        ])
...
def build(image_set, args):
    ...
    mode = 'instances'
    PATHS = {
        "train": (root / "train2017", root / f'coco_hiertext_train.json'),
        "val": (root / "val2017", root / f'coco_hiertext_val.json'),
